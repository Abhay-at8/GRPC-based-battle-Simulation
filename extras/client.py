...
def run():
    ...
    channel = grpc.insecure_channel("192.168.181.3s:50051")
    ...
    # soldier_thread.join()
    thread_exn(obj,stub,T,commander_id,N)


    
    

def thread_exn(obj,stub,T,commander_id,N):
    commander_thread = Thread(target=commander_activities, args=[obj, stub, T, commander_id])
    soldier_thread = Thread(target=soldier_action, args=[obj, stub, T, commander_id, N])
...
def soldier_action(obj, stub, T, commander_id, N):
    ...
    print("Entering soldier action section")
    ...
    print("out of Stream")
    if obj.soldierID == commander_id and obj.alive and t<T:
            print("Starting another run thread")
            thread_exn(obj,stub,T,commander_id,N)
